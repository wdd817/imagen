...
日期：2026-09-07。以下结果针对本次实际账号、端点及模型，不推断同协议的所有服务商均兼容。兼容服务测试环境的结果不等同于 OpenAI 或 Gemini 官方服务的验证结果；Vertex 平台的测试单独列出。

## 真实 API

| 服务 / 模型 | 生成 | 编辑 | 说明 |
| --- | --- | --- | --- |
| 兼容服务测试环境：Gemini 协议 / `gemini-3.1-flash-image` | 成功，1024×1024 PNG | 成功，1024×1024 PNG | 蓝色山峰改为绿色，太阳和背景保留 |
| 兼容服务测试环境：Images 协议 / `gpt-image-2` | 成功，1024×1024 PNG | 成功，1254×1254 PNG | 红杯改为青蓝色，主体与构图保留；编辑后的尺寸可能变化 |
| 兼容服务测试环境：Responses 协议 / `gpt-image-2` | 测试请求 HTTP 400 | 未验证 | 工具、direct、stream 和最小请求均返回操作不支持；无版本前缀路径为 HTTP 405 |
...
生成产物经过格式、尺寸检查及目视验证。兼容服务测试环境的 Images 缓存响应也通过 SDK adapter 的本地回放解析，未为解析检查重复生图。项目 ID、凭据、完整 API 响应及私人工作区路径不纳入公开验证记录。

Responses adapter 的工具/兼容格式通过真实 SDK 与本地假服务的契约测试，但兼容服务测试环境中的上述模型组合尚未通过。为具体服务配置 profile 时，未验证能力应标为 unknown；其他协议的成功结果不能用于启用该能力。不会自动修改用户选择的模型或协议。
